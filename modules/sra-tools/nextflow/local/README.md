# sra-tools / nextflow / local — 自定义 Nextflow 模块
...
> 需要时建议把 native 的三个子命令（prefetch / fasterq-dump / fastq-dump）封装为
> `SRATOOLS_PREFETCH` / `SRATOOLS_FASTERQDUMP` / `SRATOOLS_FASTQDUMP` 三个 process，
> fastq-dump 参数对齐 nanoseq 脚本：`--split-3 --gzip -O <outdir>`。
