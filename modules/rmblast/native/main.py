#!/usr/bin/env python3
"""rmblast（RMBlast：RepeatMasker 兼容版 NCBI BLAST+）native 标准入口驱动。
...
两个子命令分别包装 RMBlast 套件的对应程序：
- makeblastdb  建核苷酸/蛋白 BLAST 库（-in/-dbtype/-out）
- rmblastn     面向 RepeatMasker 的核苷酸 vs 核苷酸 局部比对搜索
               （-query/-db/-out/-outfmt/-evalue；RMBlast 特有引擎）

注：RMBlast 是 NCBI BLAST+ 的补丁分支，rmblastn 专为 RepeatMasker / RepeatModeler
    的重复序列搜索优化；需要标准 NCBI BLAST+（blastn/blastp 等）时见 modules/blast。
"""
...
SUBCOMMANDS = {
    ...
    "rmblastn": "rmblastn：面向 RepeatMasker 的核苷酸 query 比对到核苷酸库（-query/-db，默认 -outfmt 6 表格）",
}
...
class RmblastSkill(base.SkillBase):
    software = "rmblast"
    binary = "rmblastn"

    def __init__(self, meta_path: str | Path | None = None):
        # 单 meta 模式下 meta.yaml 位于软件级 modules/rmblast/meta.yaml（不在 native/ 下），
    ...
    def _resolve_tool(self, tool: str) -> str:
        """解析子命令对应的 RMBlast 可执行文件，带清晰报错。"""
        path = shutil.which(tool)
        if not path:
            raise RuntimeError(
                f"未找到可执行文件 '{tool}'，请先通过 Conda/Docker/Apptainer 安装 "
                "（bioconda rmblast（RMBlast）会提供 rmblastn/makeblastdb/blastdbcmd 等全套二进制）。"
            )
        return path

    def build_command(self, subcommand: str, **kw) -> list[str]:
        """根据子命令与参数构建 RMBlast 命令行。"""
        ...
        elif subcommand == "rmblastn":
            query = kw.get("query")
            if not query:
                raise RuntimeError("rmblastn 需要 -query（查询序列 FASTA）")
            db = kw.get("db")
            if not db:
                raise RuntimeError("rmblastn 需要 -db（BLAST 库前缀）")
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="rmblast-skill",
        description="rmblast（RMBlast）native 技能驱动（自动线程/TMPDIR；建库 + rmblastn 重复序列搜索）",
    )
    ...
    pm.add_argument("--dbtype", default="nucl", choices=list(_DB_TYPES),
                    help="库类型：nucl（默认，供 rmblastn）| prot（供蛋白库）")
    ...
    # rmblastn: rmblastn -query <q> -db <db> [-out <o>] [-outfmt 6] [-evalue E]
    pt = sub.add_parser("rmblastn", help=SUBCOMMANDS["rmblastn"])
    ...
    pt.add_argument("--extra-args", dest="extra_args", help="透传给 rmblastn 的额外参数（高级用法，慎用）")
