#!/usr/bin/env python3
"""生成 rmblast（RMBlast）native 测试用的合成数据。

产出（<outdir> 下）：
  ref_nucl.fa   核苷酸参考库（两条 contig：1000 bp / 800 bp）
  query_nucl.fa 单条核苷酸查询（ref_nucl 第一条 contig 的 120 bp 精确子串）
  ref_prot.fa   蛋白参考库（两条蛋白：200 aa / 160 aa；用于 makeblastdb --dbtype prot 覆盖）

说明：查询取参考的精确子串，保证能在小库上产生显著命中；序列用固定随机种子生成，
避免低复杂度导致 rmblastn 默认 dust/masking 过滤意外屏蔽。
"""
...
def main() -> int:
    ...
    # 蛋白库（仅用于覆盖 makeblastdb --dbtype prot 建库路径）
    rng = random.Random(7)
