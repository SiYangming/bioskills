# isoseq3 / snakemake / snakemake-wrappers

官方 [Snakemake Wrappers](https://github.com/snakemake/snakemake-wrappers) 中 `bio/isoseq3/*` 的引用说明与桥接脚本。

> ⚠️ **本目录仅为说明 + Schema 挂载层 + 本地桥接脚本**。真正的 wrapper 由
> Snakemake 运行时根据 `wrapper: "vX.Y.Z/bio/isoseq3/<subcommand>"` 句柄从中央仓库或
...
```bash
python wrapper.py refine --tag v3.13.0
...
并在软件级 `meta.yaml` 的 `implementations` 登记 `isoseq3_snakemake_local`。
