#!/usr/bin/env python3
"""gnu_sort / snakemake / snakemake-wrappers 本地桥接脚本。
...
WRAPPER_BASE = "bio/gnu"
DEFAULT_TAG = "v3.13.0"
OFFICIAL_AVAILABLE = False  # 2026-08 核对：bio/gnu 404

SUBCOMMAND_DEFAULTS = {
    "sort": {"threads": 8, "mem_mb": 4096},
}


class GnuSortWrapperBridge:
    ...
    def wrapper_path(self, subcommand: str = "sort") -> str:
        """返回官方句柄（仅说明用；当前官方缺失，不可被 Snakemake 解析）。"""
        return f"{self.tag}/{WRAPPER_BASE}/{subcommand}"

    def defaults(self, subcommand: str = "sort") -> dict:
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="gnu_sort snakemake-wrappers 桥接（官方缺失 → 降级 local）")
    p.add_argument("subcommand", nargs="?", default="sort", help="gnu_sort 子命令（sort）")
    ...
    bridge = GnuSortWrapperBridge(tag=args.tag)
    if args.status:
