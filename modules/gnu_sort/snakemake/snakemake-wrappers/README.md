# gnu_sort / snakemake / snakemake-wrappers

官方 [Snakemake Wrappers](https://github.com/snakemake/snakemake-wrappers) 中 GNU sort 的引用说明与降级桥接脚本。
...
**无。** `bio/gnu` 官方不存在（404），无任何 wrapper 可用。

```bash
# 核对命令（刷新时用）：
curl -s https://api.github.com/repos/snakemake/snakemake-wrappers/contents/bio/gnu
...
include: "modules/gnu_sort/snakemake/local/gnu_sort.smk"

# 之后即可使用 rule gnu_sort：
rule gnu_sort_demo:
    input:  "{filepath}"
    output: "{filepath}.sorted"
```
...
并在软件级 `meta.yaml` 的 `implementations` 登记 `gnu_sort_snakemake_local`。

## 何时选择本实现

- 目标流程语言为 **Snakemake** → 直接走 `gnu_sort_snakemake_local`（官方 wrapper 缺失）。
