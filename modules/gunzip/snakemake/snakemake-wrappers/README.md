# gunzip / snakemake / snakemake-wrappers

官方 [Snakemake Wrappers](https://github.com/snakemake/snakemake-wrappers) 中 gunzip 的引用说明与降级桥接脚本。
...
**无。** `bio/gunzip` 官方不存在（404），无任何 wrapper 可用。

```bash
# 核对命令（刷新时用）：
curl -s https://api.github.com/repos/snakemake/snakemake-wrappers/contents/bio/gunzip
...
include: "modules/gunzip/snakemake/local/gunzip.smk"
...
并在软件级 `meta.yaml` 的 `implementations` 登记 `gunzip_snakemake_local`。

## 何时选择本实现

- 目标流程语言为 **Snakemake** → 直接走 `gunzip_snakemake_local`（官方 wrapper 缺失）。
