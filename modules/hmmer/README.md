...
> 汇总说明：本模块覆盖**同一软件（HMMER）的两条版本线**，但只保留**一个 native 实现**（`native/`，`source_type: custom`、`type: native`）——3.x 现行版与 2.x 遗留版在同一驱动内以**不同子命令**区分（3.x → `hmmbuild`/`hmmpress`/`hmmsearch`；2.x → `hmmbuild2`/`hmmsearch2`，与 bioconda hmmer2 包内带 `2` 后缀的二进制命名一致）。官方 nf-core 子模块与 snakemake-wrappers 情况记录于此（不建目录，仅说明层）。安装方式见「环境安装」节，容器与 conda 环境信息记录于文末；两条版本线差异见文末「版本差异声明」。

***

## native 实现（HMMER 3.x 现行版 + 2.x 遗留版，单驱动双子命令）

# hmmer / native — profile HMM 建库与序列检索驱动

本实现为自包含驱动（`source_type: custom`、`type: native`），**一个驱动同时覆盖两条版本线**：3.x 现行版（子命令 `hmmbuild` / `hmmpress` / `hmmsearch`）与 2.x 遗留版（子命令 `hmmbuild2` / `hmmsearch2`）。二进制由**官方容器/conda**（quay.io/biocontainers/hmmer 与 hmmer2 / bioconda hmmer 与 hmmer2）或官方源码自编译提供。

## HMMER 3.x 现行版（子命令 hmmbuild / hmmpress / hmmsearch）
...
3.x 的三个子命令分别包装官方三个入口（无后缀二进制）：

## 能力

HMMER（Hidden Markov Model Toolkit）是基于隐马尔可夫模型进行蛋白质序列比对的工具集，常用于蛋白质家族鉴定、功能域识别和同源序列搜索，是 Pfam、dbCAN 等数据库的核心搜索工具。
...
## HMMER 2.x 遗留版（同一实现，子命令 hmmbuild2 / hmmsearch2）

# hmmer / native — HMMER 2.x legacy profile HMM 子命令
...
⚠️ **这是 HMMER 2.x 遗留版，不是 HMMER 3.x**：2.x 的 CLI、二进制命名与输出格式均与 3.x 不同（详见文末「版本差异声明」）。HMMER 2.x 是 **RNAmmer 1.2**（HMM 法 rRNA 基因预测，<https://services.healthtech.dtu.dk/services/RNAmmer-1.2/>，模块见 `modules/rnammer`）与**旧版 antiSMASH** 等工具的**必需运行依赖**（这些工具对 HMMER 版本敏感，须用 2.x）。**HMMER 3.x 用法见上「HMMER 3.x 现行版」章节。**

2.x 由**同一个 native 驱动**（`native/`）提供，子命令名带 `2` 后缀（`hmmbuild2` / `hmmsearch2`）以与 3.x 的同名命令区分；二进制由**官方容器/conda**（quay.io/biocontainers/hmmer2 / bioconda hmmer2）或官方源码自编译提供。

### 能力

| 子命令         | 包装命令                                                                 | 作用                                     | 线程 |
| ----------- | -------------------------------------------------------------------- | -------------------------------------- | -- |
| `hmmbuild2` | `hmmbuild [options] <hmmfile> <alignfile>`（`-n` 命名、`-F` 覆盖）           | 由多序列比对（MSA）训练 HMMER2 profile HMM        | 协议位（HMMER2 hmmbuild 无并行，不注入） |
| `hmmsearch2` | `hmmsearch [options] <hmmfile> <seqfile>`（`-E`/`-T` 阈值、`-A` 比对条数、`--cpu N`） | 用 profile HMM 检索序列库（输出走 stdout；**2.x 无 `--tblout`**） | ✅ 默认 4（注入 `--cpu`） |

### 用法

（以下命令在本模块 `native/` 目录下执行；与 3.x 子命令同目录、不同子命令名。）

```bash
# 建谱：MSA（Stockholm/SELEX/afa…）→ HMMER2 profile HMM
python main.py hmmbuild2 family.hmm family.sto -n teach_family
python main.py hmmbuild2 family.hmm family.sto -F          # -F 覆盖已存在的 HMM

# 搜索：profile HMM 检索序列库（HMMER2 无 --tblout，主输出走 stdout，可直接重定向）
python main.py hmmsearch2 family.hmm proteins.fa -E 10 --threads 4
python main.py hmmsearch2 family.hmm proteins.fa -T 5 -A 0 --threads 4 > hits.txt

# Agent / Schema 自省（3.x / 2.x 子命令一并列出）
python main.py --schema
python main.py --list-commands
```

每个子命令支持 `--threads` / `--tmpdir` 运行期覆盖（`--tmpdir` 注入 `TMPDIR` 环境变量）。`--threads` 对 `hmmsearch2` 注入 `--cpu N`；**HMMER2 `hmmbuild2` 无线程参数**，`--threads` 仅作协议位不注入。

> 💡 **二进制命名（关键）**：**bioconda hmmer2 包内所有工具带 `2` 后缀**——`hmmbuild2` / `hmmsearch2` / `hmmalign2` / `hmmcalibrate2` / `hmmconvert2` / `hmmemit2` / `hmmfetch2` / `hmmindex2` / `hmmpfam2`（bioconda recipe 用 `Makefile.in.patch` 把安装名改为 `<prog>2`，以避免与 HMMER3 冲突）；上游源码自编译为**无后缀**（`hmmbuild` / `hmmsearch`），Debian 包为 **`hmm2` 前缀**（`hmm2build` / `hmm2search`）。驱动按**多候选名探测**：`hmmbuild2→hmmbuild→hmm2build`、`hmmsearch2→hmmsearch→hmm2search`，三条命名均可用（并校验 `-h` 输出确为 `HMMER 2.`，避免误用同名的 HMMER3）。本模块的 2.x 子命令名（`hmmbuild2` / `hmmsearch2`）即采用 bioconda 的带 `2` 后缀命名。
...
HMMER 2.x 的现代主要用途是**为依赖它的遗留工具提供 HMM 引擎**——RNAmmer 1.2 的 rRNA 预测内部调用 HMMER2 `hmmsearch`（官网 <https://services.healthtech.dtu.dk/services/RNAmmer-1.2/>，安装见 `modules/rnammer`），旧版 antiSMASH 亦需要 2.x。以下为 HMMER2 原生 CLI 的典型用法；**等价能力由 `native/main.py` 的 `hmmbuild2` / `hmmsearch2` 子命令提供**（先 CLI 后 main.py，见上「用法」）。
...
| 参数 | 说明 |
| ---- | ---- |
| `-n <s>` | `hmmbuild2`：给 HMM 命名（缺省取 MSA 名） |
| `-F` | `hmmbuild2`：强制覆盖已存在的 HMM 文件（否则 HMMER 拒绝覆盖） |
| `-E <x>` | `hmmsearch2`：序列 E-value 阈值（HMMER2 默认 10.0） |
| `-T <x>` | `hmmsearch2`：序列位分阈值（默认负无穷，仅由 `-E` 控制） |
| `-A <n>` | `hmmsearch2`：限制比对输出的最优结构域条数（`-A0` 关闭比对输出） |
| `--cpu <n>` | `hmmsearch2`：CPU 并行数（HMMER2 专家选项；本驱动由 `--threads` 注入） |
| `--threads` | 协议位：`hmmsearch2` 注入 `--cpu N`；`hmmbuild2`（2.x 无并行）不注入 |
| `--tmpdir` | 协议位：注入 `TMPDIR` 环境变量 |

> 💡 **HMMER2 无 `--tblout`**（表格输出是 HMMER3 新增）；需要结构化结果时解析 stdout，或改用本模块 HMMER 3.x 子命令（`hmmsearch`）。
> 桥接句：以上 CLI 用法等价于 `python main.py hmmbuild2 <hmmfile> <alignfile> -n <name>` 与 `python main.py hmmsearch2 <hmmfile> <seqfile> -E <x> --threads <n>`。
...
> 一键安装可直接 `bash native/install.sh`（默认 `--line 3`；auto：有 conda/mamba 走 bioconda；否则拉官方源码归档到用户前缀自编译）。
...
### 5. HMMER 2.x 遗留版安装（同一 native 实现，子命令 hmmbuild2 / hmmsearch2）
...
# 或一键：bash native/install.sh --line 2   （auto：有 conda/mamba 走 bioconda；否则官方源码自编译到用户前缀）
...
> **Homebrew 无可登记公式**：homebrew-core（`formulae.brew.sh/api/formula/hmmer2.json`）与 brewsci/bio（`.../Formula/hmmer2.rb`）两源均 **404**（2026-09 核实）——homebrew-core 的 `hmmer` 公式是 **HMMER3**，非 HMMER2；故 HMMER 2.x 仅 conda / 官方容器 / 源码自编译，或本仓库 `native/install.sh --line 2`（等价上述第 (1)/(4) 条的用户级版本，含 sha256 校验 + PATH 写入）。

> 💡 也可一次装齐两条版本线：`bash native/install.sh --line both`（分别建 `hmmer-native` 与 `hmmer2` 两个 conda 环境；源码路线下两条线会先后落入不同前缀，注意 PATH 顺序）。

## 测试

```bash
# 单一 native 实现（两条版本线共用一个 run_test.sh）
cd modules/hmmer/native && bash test/run_test.sh
```

任何环境下 exit 0 并打印 `ALL TESTS PASSED`：

* **自省必跑**：`--list-commands`（断言 5 个子命令：`hmmbuild`/`hmmpress`/`hmmsearch`/`hmmbuild2`/`hmmsearch2`）与 `--schema`（JSON 合法性）。
* **契约必跑**：五子命令 `--help` 均含 `--threads` / `--tmpdir`；argv 构造断言（monkeypatch 二进制路径，覆盖 3.x 三命令 + 2.x 两命令，并断言 2.x `hmmbuild2` 不注入 `--cpu`）。
* **可选真跑**（未安装则 `[SKIP]`）：
  * HMMER 3.x：PATH 有 `hmmbuild`/`hmmpress`/`hmmsearch` → `hmmbuild` → `hmmpress`（断言 `.h3f/.h3i/.h3m/.h3p`）→ `hmmsearch`（断言 `--tblout`/`-o` 产物）；
  * HMMER 2.x：按多候选名探测（`hmmbuild2`/`hmmbuild`/`hmm2build`、`hmmsearch2`/`hmmsearch`/`hmm2search`）且校验 `-h` 输出为 `HMMER 2.` → `hmmbuild2` 建谱 → `hmmsearch2` 检索。

## 版本

* hmmer **3.4**（HMMER 3.x 现行版，默认版本线；上游 2023-08 发布；bioconda 现行 `hmmer=3.4`，linux-64 build `h7d74f8d_5`，2026-08 起发布）

* hmmer2 **2.3.2**（HMMER 2.x 遗留版，同一 `native/` 实现的 2.x 子命令 `hmmbuild2`/`hmmsearch2`；bioconda 现行/唯一版本，linux-64 build `h87e0c26_12`，2017-05 起发布；二进制带 `2` 后缀）
...
* nf-core 官方**无** `modules/nf-core/hmmer2`（2026-09 在线核实 404；nf-core 仅登记 HMMER3 的 `modules/nf-core/hmmer`）。抓取：`curl -s https://api.github.com/repos/nf-core/modules/contents/modules/nf-core/hmmer2` → `{"message":"Not Found","status":"404"}`。Nextflow 场景直接调用 `native/main.py`（2.x 子命令 `hmmbuild2`/`hmmsearch2`）兜底。
* snakemake-wrappers 官方**无** `bio/hmmer2`（2026-09 在线核实 404；`bio/` 仅有 HMMER3 的 `bio/hmmer`）。抓取：`curl -s https://api.github.com/repos/snakemake/snakemake-wrappers/contents/bio/hmmer2` → 404。Snakemake 场景以 `native/` 宿主机驱动（2.x 子命令）兜底。

> 本模块未建 `nextflow/`、`snakemake/` 目录；HMMER2 无官方引擎层，两引擎场景均由 `native/` 驱动（2.x 子命令）兜底。

## 版本差异声明（两条版本线 + 各引擎实现）

| 实现                      | 版本行     | 版本      | 来源 / 说明                                                                        |
| ----------------------- | ------- | ------- | ------------------------------------------------------------------------------ |
| `hmmer_native`（native/）   | HMMER 3.x | **3.4** | official biocontainer：quay.io/biocontainers/hmmer:3.4--h7d74f8d\_5 / bioconda hmmer=3.4；子命令 hmmbuild/hmmpress/hmmsearch；默认版本线 |
| `hmmer_native`（native/）   | HMMER 2.x | **2.3.2** | official biocontainer：quay.io/biocontainers/hmmer2:2.3.2--h87e0c26\_12 / bioconda hmmer2=2.3.2；子命令 hmmbuild2/hmmsearch2（同一实现内的遗留版本线） |
...
| 维度 | HMMER **3.x**（子命令 `hmmbuild`/`hmmpress`/`hmmsearch`） | HMMER **2.x** 遗留版（子命令 `hmmbuild2`/`hmmsearch2`） |
| ---- | -------------------------------- | ------------------------------ |
| 版本 | 3.4 | 2.3.2 |
...
| 子命令 | `hmmbuild` / `hmmpress` / `hmmsearch`（+ hmmscan/phmmer/jackhmmer 等） | `hmmbuild2` / `hmmsearch2`（+ hmmpfam/hmmalign/hmmcalibrate…） |
| 表格输出 | `--tblout <f>`（每命中一行） | **无 `--tblout`**（报告走 stdout，需自行解析） |
| 建库按压 | `hmmpress` → `.h3f/.h3i/.h3m/.h3p` | **无 hmmpress**（2.x 无按压索引流程） |
| 并行（建谱） | `hmmbuild --cpu N` | `hmmbuild2` **无并行**（`hmmsearch2` 支持 `--cpu N`） |
...
> 选择建议：新场景优先 HMMER 3.x 子命令（`hmmbuild`/`hmmpress`/`hmmsearch`）；仅当依赖方（RNAmmer 1.2、旧版 antiSMASH 等）明确要求 HMMER 2.x 时才用 `hmmbuild2`/`hmmsearch2`。
...
```yaml
# hmmer native Conda 环境配方（HMMER 3.x 版本线，HPC 无 root / 非容器兜底）
...
```yaml
# hmmer native Conda 环境配方（HMMER 2.x 遗留版版本线，HPC 无 root / 非容器兜底）
...
dependencies:
  - python=3.11
  - hmmer2=2.3.2          # 二进制带 2 后缀（hmmsearch2/hmmbuild2），对应子命令 hmmbuild2/hmmsearch2
  - pyyaml>=6.0
```
...
* 安装方式（本地）：3.x `mamba create -n hmmer-native -c conda-forge -c bioconda hmmer=3.4`（或 `brew install hmmer`）；2.x `mamba create -n hmmer2 -c conda-forge -c bioconda hmmer2=2.3.2`（或 `bash native/install.sh --line 2`）
