# transdecoder / nextflow / local — 自定义 Nextflow 模块
...
> 需要时建议把 native 的命令逻辑（`TransDecoder.LongOrfs -t <fa> -O <dir> [--gene_trans_map] [-m] [-G] [-S]`
> 与 `TransDecoder.Predict -t <fa> -O <dir> [--retain_pfam_hits] [--retain_blastp_hits] --cpu <N>`）封装为 process。
