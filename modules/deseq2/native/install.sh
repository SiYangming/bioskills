#!/usr/bin/env bash
# =============================================================================
# install.sh — deseq2（DESeq2 R/Bioconductor 包）宿主机本地安装脚本
#
# 归属    ：bioskills modules/deseq2/native/install.sh（native 实现安装方式）
# 迁移形态：现代规范（官方镜像优先；DESeq2 无独立二进制 → 仅 conda / R 两条路线）
#   - conda 路线（默认优先）：mamba/conda 建独立环境，pin bioconda::bioconductor-deseq2
#   - R 路线（无 conda 兜底）：BiocManager::install("DESeq2")（R/Bioconductor 原生）
#   - 版本默认 1.50.2，与 modules/deseq2/meta.yaml software_versions.native 对齐
#
# 官方来源：
#   homepage : https://bioconductor.org/packages/DESeq2/
#   bioconda : https://anaconda.org/bioconda/bioconductor-deseq2
#   (容器：quay.io/biocontainers/bioconductor-deseq2；用法见模块 README)
#
# 用法示例：
#   bash install.sh                                   # auto：有 conda 走 bioconda，否则 BiocManager
#   bash install.sh --method conda                    # 强制 conda（默认建独立 env: deseq2）
#   bash install.sh --method R                        # 强制 BiocManager（需本机 Rscript）
#   bash install.sh --conda-env diffexpr --force      # 指定环境名 / 已存在时强制重建
#   bash install.sh --version 1.50.2                  # 覆盖版本（安装后版本断言对齐）
# =============================================================================
set -euo pipefail

DEFAULT_VERSION="1.50.2"
...
# ---------------- 路线 conda：独立 env（bioconductor-deseq2） ----------------
install_conda() {
    [[ -n "$CONDA_BIN" ]] || die "--method conda 但未检测到 mamba/conda（PATH 中）"
    local env_exists
    log "使用 conda 安装 ${CONDA_PKG}=${VERSION} 到环境: $CONDA_ENV"
