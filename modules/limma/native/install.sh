#!/usr/bin/env bash
# =============================================================================
# install.sh — limma（limma R/Bioconductor 包）宿主机本地安装脚本
#
# 归属    ：bioskills modules/limma/native/install.sh（native 实现安装方式）
# 迁移形态：现代规范（官方镜像优先；limma 无独立二进制 → 仅 conda / R 两条路线）
#   - conda 路线（默认优先）：mamba/conda 建独立环境，pin bioconda::bioconductor-limma
#   - R 路线（无 conda 兜底）：BiocManager::install("limma")（R/Bioconductor 原生）
#   - 版本默认 3.66.0，与 modules/limma/meta.yaml software_versions.native 对齐
#
# 官方来源：
#   homepage : https://bioconductor.org/packages/limma/
#   bioconda : https://anaconda.org/bioconda/bioconductor-limma
#   (容器：quay.io/biocontainers/bioconductor-limma；用法见模块 README)
#
# 用法示例：
#   bash install.sh                                   # auto：有 conda 走 bioconda，否则 BiocManager
#   bash install.sh --method conda                    # 强制 conda（默认建独立 env: limma）
#   bash install.sh --method R                        # 强制 BiocManager（需本机 Rscript）
#   bash install.sh --conda-env diffexpr --force      # 指定环境名 / 已存在时强制重建
#   bash install.sh --version 3.66.0                  # 覆盖版本（安装后版本断言对齐）
# =============================================================================
set -euo pipefail

DEFAULT_VERSION="3.66.0"
VERSION="$DEFAULT_VERSION"
CONDA_ENV="limma"
METHOD="auto"          # auto | conda | R
FORCE=0

CONDA_PKG="bioconductor-limma"
R_PKG="limma"
...
install_conda() {
    ...
    local env_exists
    log "使用 conda 安装 ${CONDA_PKG}=${VERSION}（含 edgeR 依赖）到环境: $CONDA_ENV"
