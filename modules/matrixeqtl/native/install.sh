#!/usr/bin/env bash
# =============================================================================
# install.sh — matrixeqtl（Matrix eQTL R 包）宿主机本地安装脚本
#
# 归属    ：bioskills modules/matrixeqtl/native/install.sh（native 实现安装方式）
...
#   - 版本默认 2.3，与 modules/matrixeqtl/meta.yaml software_versions.matrixeqtl_native 对齐；
#     CRAN 已于 2026-09 发布 2.4，install.packages() 实际安装以 CRAN 当前版为准（接口一致）。
#
# 官方来源：
#   CRAN     : https://cran.r-project.org/package=MatrixEQTL
#   conda r 频道 : https://anaconda.org/channels/r/packages/r-matrixeqtl/overview（conda 包名带 r- 前缀）
...
#   bash install.sh --method conda                    # 强制 conda 路线（建独立 env: matrixeqtl）
