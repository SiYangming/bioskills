#!/bin/bash
# 归档说明（2026-09）：
#  - 上游：https://github.com/ewels/AWS-iGenomes（bucket 仍公开可用；本副本为 2018 年前后快照，
#    基因组/索引列表更新以 ewels.github.io/AWS-iGenomes 为准）。
#  - 数据源：AWS S3 公共 bucket s3://ngi-igenomes/igenomes（eu-west-1，无需账户，
#    aws s3 ls --no-sign-request s3://ngi-igenomes/ 可验证）。
#  - 用途：按 基因组/来源/build/类型 四层筛选并同步 illumina iGenomes 参考序列与索引。
#  - 用法：bash aws-igenomes.sh -g Homo_sapiens -s Ensembl -b GRCh37 -t fasta -o ./ref
