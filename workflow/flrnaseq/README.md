# flrnaseq — 全长 RNA-seq ORF 预测复合流程
...
| 阶段           | 软件技能                          | 主要入口                                  |
| ------------ | ----------------------------- | ------------------------------------- |
| TransDecoder | `modules/transdecoder/native/` | `python main.py longorfs/predict ...` |
...
| ORFanage 实现         | `modules/orfanage/`（独立模块）                                                   | 原 orfrange\_archive/ 已归位为 orfanage 技能（native/legacy 存原始文件）                        |
| 批处理脚本               | `modules/td2/native/legacy/run_td2_orf_prediction.sh` + `LEGACY_run_smk.sh` | 原 flrnaseq.sh/run\_td2\_orf\_prediction.sh / run\_smk.sh                          |
