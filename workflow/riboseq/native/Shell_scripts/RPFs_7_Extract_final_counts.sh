...
do
python3 $PYTHON_SCRIPTS_DIR/counting_script.py -bam $BAM_dir/${filename}_pc_deduplicated_sorted.bam -fasta $most_abundant_fasta -len $lengths -offset $offsets -out_file ${filename}_pc_final.counts -out_dir $counts_dir &
done
wait
