...
```bash
python custom/nanoseq/nanoseq.py \
    --samplesheet samples.csv --reference ref.fa --gtf ref.gtf \
    --outdir results --orf-tool transdecoder

python custom/nanoseq/nanoseq.py ... --real            # 真实执行
python custom/nanoseq/nanoseq.py ... --with-prep       # 启用 SRA 下载
python custom/nanoseq/nanoseq.py ... --with-dorado     # 启用碱基识别
```
