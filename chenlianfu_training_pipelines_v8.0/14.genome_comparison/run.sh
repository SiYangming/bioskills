...
DATE=`date +%Y%m%H%M%S`
User=`whoami`
perl -p -e 's/^dbLogin=.*/dbLogin=train/; s/^dbPassword=.*/dbPassword=123456/' /opt/biosoft/orthomclSoftware-v2.0.9/doc/OrthoMCLEngine/Main/orthomcl.config.template > orthomcl.config
echo "perl -p -i -e 's/:3307/_${DATE}_$User/' orthomcl.config" | sh

# 2. 安装 OrthoMCL 运行所需要的 mysql 数据库的表
echo "CREATE DATABASE IF NOT EXISTS orthomcl_${DATE}_$User" | mysql -utrain -p123456
...
do
    ...
    for i in `cat ../orthologGroups.txt`
    do
        echo "calculating_omega_by_yn00.pl --omega_for_PSG 1.0 --input_tree ../tree.txt --target_branch_species $y ../orthologGroups_CDS/$i.phy > dnds_yn00/$i.txt 2> dnds_yn00/$i.stats"
    done > command.calculating_omega_by_yn00.list
    ...
    for i in `cat ../orthologGroups.txt`
    do
        echo "calculating_omega_by_codeml.pl --omega_for_PSG 1.0 --target_branch_species $y ../orthologGroups_CDS/$i.phy ../tree.txt > dnds_ML/$i.txt 2> dnds_ML/$i.stats"
    done > command.calculating_omega_by_ML.list
...
# 删除大量小文件
# rm -rf orthologGroups_CDS/*.tmp
# (3) 使用branch-site model对候选基因进行正选择分析
for x in `cat branches_innerNode.txt branches_outerNode.txt`
do
    y=$x
    x=${x/:*/}
    y=${y/*:/}
    ...
    do
        echo "paml_branch-site_model_analysis.pl --target_branch_species $y orthologGroups_CDS/$i.phy tree.txt BS_$x/branch-site_model/$i > BS_$x/branch-site_model/$i.p"
...
do
    ...
    cd ..
done | grep -v NULL > results.PSG.branches_innerNode
perl -e 'while (<>) { @_ = split /\s+/; push @stats, $_[0] unless exists $stats{$_[0]}; $stats{$_[0]} ++; } foreach (@stats) { print "$_\t$stats{$_}\n"; }' results.PSG.branches_innerNode > results.PSG.branches_innerNode.stats
for x in `cat branches_outerNode.txt`
do
    y=$x
    x=${x/:*/}
    y=${y/*:/}
    ...
    perl -e 'while (<branch-site_model/*.p>) { my $ocg = $_; $ocg =~ s/.*\///; $ocg =~ s/.p//; open IN, $_; my $p = "NULL\n"; while (<IN>) { chomp; @_ = split /\t/; $p = "$_[0]\n" if ($_[0] <= 0.05 && ($_[1] eq "BEB_Significance_YES")); } print "$ARGV[0]\t$ocg\t$p"; }' $x
    cd ..
done | grep -v NULL > results.PSG.branches_outerNode
perl -e 'while (<>) { @_ = split /\s+/; push @stats, $_[0] unless exists $stats{$_[0]}; $stats{$_[0]} ++; } foreach (@stats) { print "$_\t$stats{$_}\n"; }' results.PSG.branches_outerNode > results.PSG.branches_outerNode.stats
cd ..
