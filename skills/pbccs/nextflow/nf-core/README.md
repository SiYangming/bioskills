# pbccs / nextflow / nf-core

官方 [nf-core/modules](https://github.com/nf-core/modules) 中 pbccs 模块的引用说明。
...
nf-core pbccs 是**单模块**（目录下仅 `environment.yml` / `main.nf` / `meta.yml` / `tests`，无子目录）：

| 子模块 | 作用 |
|--------|------|
| `pbccs` | subreads BAM → HiFi/CCS BAM（ccs 全流程，支持 chunk 分块） |

## 在 Nextflow DSL2 中使用

```groovy
// 1. 在项目目录安装模块到 modules/nf-core/
//   nf-core modules install nf-core pbccs

// 2. include 使用
include { PBCCS } from './modules/nf-core/pbccs/main'

workflow {
    PBCCS ( [ [id: 'sample1'], subreads_bam, subreads_pbi, chunk_num, chunk_on ] )
}
```
...
```bash
curl -s https://api.github.com/repos/nf-core/modules/contents/modules/nf-core/pbccs \
...
并在软件级 `meta.yaml` 的 `implementations` 登记 `pbccs_nextflow_local`。
