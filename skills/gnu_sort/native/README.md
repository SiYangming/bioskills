...
`snakemake.smk/isoseq.smk/workflow/skills/gnu_sort/snakemake/local/gnu_sort.smk`（`sort [args] <in> > <out>.sorted`）。
...
- 与 isoseq.smk 对应关系：`sort` ← `workflow/skills/gnu_sort/snakemake/local/gnu_sort.smk`（`sort_gtf` 的通用版）。
