...
### 并发批处理脚本（run_bamtools_convert.sh）
...
4) BamTools convert（目录批处理并发，默认转 FASTA）
```bash
...
### 并发批处理脚本（run_tama_polyacleanup.sh）
...
2) 在 index 之前对 GTF 排序（写入 INDEX 目录；支持 `.gtf.gz`）：
```bash
...
3) 生成 ULTRA 索引（一次性）。建议把索引写入独立目录（支持 `.fa.gz/.fasta.gz` 自动解压）：
```bash
...
4) 使用 ULTRA 进行比对（生成 BAM，支持 `.fa.gz/.fasta.gz` 自动解压）
```bash
