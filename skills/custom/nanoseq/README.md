# custom/nanoseq — Nanopore RNA-seq 复合流程
...
python skills/custom/nanoseq/nanoseq.py ... --real            # 真实执行
python skills/custom/nanoseq/nanoseq.py ... --with-prep       # 启用 SRA 下载
python skills/custom/nanoseq/nanoseq.py ... --with-dorado     # 启用碱基识别
```
...
| 原 workflow 快照（去重后） | `workflow_skeleton/LEGACY_WORKFLOW/` | Snakefile + 原始 rules/（含 common.smk，比模块迁移版更完整）；scripts/envs 因已有等价副本已清理 |
