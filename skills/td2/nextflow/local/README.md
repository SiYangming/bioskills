# td2 / nextflow / local — 自定义 Nextflow 模块
...
> 需要时建议把 native 的命令逻辑（`TD2.LongOrfs -t <fa> -O <dir> [--gene-trans-map] [-m] [-M] [-G] [-S]`
> 与 `TD2.Predict -t <fa> -O <dir> [--retain-mmseqs-hits] [--retain-blastp-hits] [--retain-hmmer-hits] --threads <N>`）封装为 process。
